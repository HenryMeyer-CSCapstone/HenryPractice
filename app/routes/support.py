...
from app.classes.data import Support
from app.classes.forms import SupportForm
...
def supportNew():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('support',supportID=newSupport.id))

    # if form.validate_on_submit() is false then the user either has not yet filled out
    # the form or the form had an error and the user is sent to a blank form. Form errors are 
    # stored in the form object and are displayed on the form. take a look at blogform.html to 
    # see how that works.
    return render_template('supportform.html',form=form)

@app.route('/support/<supportID>')
# This route will only run if the user is logged in.
@login_required
def support(supportID):
    # retrieve the blog using the blogID
    thisSupport = Support.objects.get(id=supportID)
    ...
    theseComments = Support.objects(support=thisSupport)
    # Send the blog object and the comments object to the 'blog.html' template.
    return render_template('support.html',support=thisSupport,comments=theseComments)

@app.route('/support/edit/<supportID>', methods=['GET', 'POST'])
@login_required
def supportEdit(supportID):
    editSupport = Support.objects.get(id=supportID)
    ...
    if current_user != editSupport.author:
        flash("You can't edit a support you don't own.")
        return redirect(url_for('support',supportID=supportID))
    # get the form object
    form = SupportForm()
    ...
    form.fullname.data = editSupport.fullname
    form.gender.data = editSupport.gender
    form.ageage.data = editSupport.ageage
    form.aboutme.data = editSupport.aboutme
...
def supportDelete(supportID):
    ...
    if current_user == deleteSupport.author:
        ...
        flash('Your Support was deleted.')
    else:
        # if the user is not the author tell them they were denied.
        flash("You can't delete a support you don't own.")
